    ...
    for i in range(10):            
        if (min1[i] <= 5 or min2[i] <= 5):
            if (abs(min1[i] - min2[i]) <= 10):
                return i
    for i in range(10):
        if (min1[i] <= 16 or min2[i] <= 16):
            if (abs(min1[i] - min2[i]) <= 16):
                return i
...
def Pic_Reg(image_name=None):
    ...
    #im.show()
                    #all by pixel
    s = 18          #start postion of first number
    w = 14          #width of each number
    h = 32          #end postion from top
    t = 8          #start postion of top
    im_new = []
